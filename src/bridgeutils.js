...
const StatusColors = {
    Away: [0.1714, 0.3545],
    Occupied: [0.6802, 0.3106],
    Available: [0.1561, 0.1587],
    Normal: [0.4575, 0.4099],
};
...
async function fanSetOn(on) {
    ...
    try {
        ...
        log.error(
            'fanSetOn: HUE fanOn was not able to get an authenticated API'
        );
    }

    try {
        const setFan = await authenticatedApi.lights.setLightState(
            config.fanId,
            fanState
        );
